...
<%
	...
	String fenlei=Common.toChineseAndTrim(request.getParameter("fenlei"));
	String sql = "select * from news where fenlei='"+fenlei+"' and  title like '%"+title+"%' order by id desc";
...
<TABLE class=dragTable cellSpacing=0 cellPadding=0 width="100%" border=0>
		...
		<TD  class=middle align="left" >
               <TABLE class=xsnr id=tb1_nr1  cellSpacing=0 cellPadding=0 width="100%" border=0>
                    <TBODY>
                    <TR>
          <TD  align=left><%=fenlei%></TD>
          <TD align=center>发布时间</TD>
          <TD align=center>发布人</TD>
		  <TD align=right>点击率</TD>
		</TR>
                    <%List nnliet=news.get1ComList(sql);
                    if(!nnliet.isEmpty()){
